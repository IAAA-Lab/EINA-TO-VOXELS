...
# pcloudproc.py - Point cloud processing.
...
class SparseMatrix():
    ...
    {(i,j,k):value} where value can be any object (but
    all values should be of the same type).
    Typically this will be an integer with the number of points
    included in the cell, but it can be used as an id or to store
    other things.
    Cells with a "zero" (zero points, no id, null object etc.) do
    not have an entry in this dictionary.
    ...
    def create_from_coords(cls, coords_iterator, resolution, bcube):
        ...
        be an integer with the number of points in the coords_iterator that fall in that
        cell.
...
def cells_to_POV(cluster_list, matrix, filepath, tolerance,
    ...
    to follow POV axis conventions as OSM2POV does. It also assumes that
    the values in matrix are integers.
    """
                ...
                if meld_boxes:
                    ...
                    for box in boxes:
                        ...
                        if cell_type == "sphere":
                            ...
                            raise ValueError(
                                "cell_type " + cell_type + " is not valid for POV output when melding cells")
                    ...
                    for cell in cells_in_tolerance:
                        ...
                        if cell_type == "box":
                            ...
                            raise ValueError(
                                "cell_type " + cell_type + " is not valid for POV output when not melding cells")
                    f.write('  texture {texture_VOXEL}\n')
                    # Using no_shadow prevents the "voxels" from casting shadows
                    # If this is or not useful, will depende on the final illumination
                    # of the scene
...
def cells_to_Threejs(cluster_list, matrix, filepath, tolerance,
    ...
    to follow its axis conventions. It also assumes that the values
    in matrix are integers.
    """
...
def main():
    ...
    if args.resolution:
        ...
        matrix = SparseMatrix.create_from_coords(simple_text_file_generator(args.input_file,
                                                                            coord_filter,
                                                                            coord_transformer=lambda
                                                                                coords: coords_to_hack_map_proj(coords,
                                                                                                                args.origin_lon,
                                                                                                                args.origin_lat,
                                                                                                                args.bcube[
                                                                                                                    2])),
