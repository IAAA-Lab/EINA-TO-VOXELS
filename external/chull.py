# This is a code sample by https://michelanders.blogspot.com.es/2012/02/3d-convex-hull-in-python.html
# © Michel J. Anders
# Creative Commons License, CC BY-SA
# It is based on the algorithm in C in Joseph O'Rourke's incremental 3D convex hull algorithm 
# from his book Computational Geometry in C, but it isues floating point numbers instead of integers
...
endfacet"""%(self.vertex[0],self.vertex[1],self.vertex[2])
	...
	def MakeCcw(self,e,p):
		...
		by the code, the following condition is maintained for each face self:
		one of the two endpoints of self.edge[i] matches self.vertex[i]. 
		But note that this does not imply that self.edge[i] is between
		self.vertex[i] and self.vertex[(i+1)%3].  (Thanks to Bob Williamson.)
		"""
		...
		# Set vertex[0] & [1] of self to have the same orientation
		# as do the corresponding vertices of fv
		i=0
		while(fv.vertex[i] != e.endpts[0]):
			i+=1
		# Orient self the same as fv
